...
customElements.define('lit-credit-calculator', class LitCreditCalculator extends LitElement {
    static get properties() {
        return {
            creditAmount: { type: Number },
            loanInterest: { type: Number },
            timeCredit: { type: Number },
            date: { type: String },
            monthlyPayment: { type: Number },
            result: { type: Number },
            resultCreditAmount: { type: Number },
    ...
    connectedCallback() {
        ...
        this.creditAmount = 1000000;
        this.loanInterest = 10;
        this.timeCredit = 60;
        this.monthlyPayment = 0;
        this.result = 0;
        this.resultCreditAmount = 0;
        ...
        return html`
            <div  style="display:flex;">
                <div class="container" style="min-width:340px;max-width:340px;margin:8px;font-size:large;">
                    ...
                    <div class="input"><span>Сумма кредита</span><input id="creditAmount" value="${this.creditAmount}" /></div>
                    <div class="input"><span>Проценты (% за год)</span><input id="loanInterest" type="number" value="${this.loanInterest}" /></div>
                    <div class="input"><span>Срок кредита (месяц)</span><input id="timeCredit" type="number" value="${this.timeCredit}" /></div>
                    <div class="input"><span>Дата выдачи кредита</span><input id="date" type="date" value="${this.date}" /></div>
                    <div class="input"><div class="flex"></div><button @click="${this._calc}">Обновить</button></div>
                ...
                <lit-chart type="bar" .data="${this.data}" .options="${this.options}" style="flex:1;border: 1px solid lightgray;margin:8px;padding: 8px"></lit-chart>
            </div>
    ...
    _calc() {
        ...
        this._dataMonth = [];
        let prev = 0;
        let d = this.renderRoot.getElementById('date').value ? new Date(this.renderRoot.getElementById('date').value) : this.date || new Date();
